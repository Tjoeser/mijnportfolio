...
public function Sendemail($userEmail, $userFullName, $content)
{
    $mail = new PHPMailer(true);

    try {
        // SMTP settings for StableHost
        $mail->isSMTP();
        $mail->Host       = APP_HOST;          // e.g., mail.thijsrietveld.com (check cPanel)
        $mail->SMTPAuth   = true;
        $mail->Username   = APP_UN;            // your full email
        $mail->Password   = APP_PW;            // email password / app password
        $mail->SMTPSecure = 'ssl';             // SSL is usually more reliable on StableHost
        $mail->Port       = 465;               // SSL port
        $mail->Timeout    = 30;                // increase timeout in case of slow connection

        // Sender & recipient
        $mail->setFrom(APP_EMAIL, 'Website Contact Form'); // must be your domain email
        $mail->addReplyTo($userEmail, $userFullName);      // user submitting form
        $mail->addAddress('thijs0302@gmail.com', 'Thijs Rietveld'); // your Gmail

        // Email content
        $mail->isHTML(false);
        $mail->Subject = 'Contact Form Submission from ' . $userFullName;
        $mail->Body    = "From: $userFullName <$userEmail>\n\n" . $content;
